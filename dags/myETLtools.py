...
def transform(df):
    ...
    engine = create_engine("postgresql://postgres:postgres@localhost:5432/airflow")
    ...
    for dim in dims:
        exec(f'{dim} = spark.createDataFrame(pd.read_sql("SELECT * FROM {dim}", engine))')
    ...
    # Create Fact table from df
    # rearrange columnstion_date", 
    # df = (df.withColumn("item_quantity", F.col("item_quantity").cast("integer")) 
